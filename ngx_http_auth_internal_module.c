...
static ngx_int_t ngx_http_auth_internal_proxy_fingerprint_variable(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
char *ngx_http_auth_internal_set_request_secrets_slot(ngx_conf_t *cf,
...
static ngx_int_t ngx_http_auth_internal_deny(ngx_http_request_t *r,
    ngx_http_auth_internal_ctx_t *ctx, const char *log_message,
...
static ngx_command_t ngx_http_auth_internal_commands[] = {
    { ngx_string("auth_internal"),
      ...
      offsetof(ngx_http_auth_internal_conf_t, enable),
      NULL },
      
    { ngx_string("auth_internal_request_secrets"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE1234,
      ngx_http_auth_internal_set_request_secrets_slot,
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_auth_internal_conf_t, request_secrets),
      NULL },

    { ngx_string("auth_internal_proxy_secret"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_auth_internal_conf_t, proxy_secret),
      NULL },
    ...
    { ngx_string("auth_internal_empty_deny"),
      ...
      ngx_conf_set_flag_slot,
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_auth_internal_conf_t, empty_deny),
      NULL },
    ...
    { ngx_string("auth_internal_failure_deny"),
      ...
      ngx_conf_set_flag_slot,
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_auth_internal_conf_t, failure_deny),
      NULL },
    ...
    { ngx_string("auth_internal_timeout"),
      ...
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_auth_internal_conf_t, timeout),
      NULL },
    ...
    { ngx_string("auth_internal_header"),
      ...
      ngx_conf_set_str_slot,
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_auth_internal_conf_t, header_name),
      NULL },
...
static ngx_http_variable_t  ngx_http_auth_internal_vars[] = {
    ...
    { ngx_string("auth_internal_proxy_fingerprint"), NULL,
      ngx_http_auth_internal_proxy_fingerprint_variable,
      0, 0, 0 },
...
static ngx_int_t
ngx_http_auth_internal_proxy_fingerprint_variable(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    ngx_http_auth_internal_conf_t *conf;
    ...
    computed_md5 = ngx_http_auth_internal_compute_md5_hex(r,
...
char *
ngx_http_auth_internal_set_request_secrets_slot(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf)
{
    char  *p = conf;
...
static ngx_int_t
ngx_http_auth_internal_deny(ngx_http_request_t *r,
    ngx_http_auth_internal_ctx_t *ctx, const char *log_message,
    ngx_uint_t deny_flag)
{
    if (deny_flag) {
...
ngx_http_auth_internal_handler(ngx_http_request_t *r)
{
    ...
    if (h == NULL) {
        ...
        return ngx_http_auth_internal_deny(r, ctx,
    ...
    if (conf->request_secrets == NULL || conf->request_secrets->nelts == 0) {
        ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
            "auth internal skipped due to no configured secret");
...
ngx_http_auth_internal_create_srv_conf(ngx_conf_t *cf)
{
    ...
    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_auth_internal_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
static char *
ngx_http_auth_internal_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_auth_internal_conf_t *prev = parent;
    ngx_http_auth_internal_conf_t *conf = child;
